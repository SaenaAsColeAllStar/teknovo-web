...
import { TEKNOVO_SIGN_IN_PATH } from "@/lib/auth-sign-in-path";
import { BRAND_SHORT } from "@/lib/branding";
import { cn } from "@/lib/utils";

const AFTER_RESET = "/dashboard";

/** Existing public page — no dedicated terms URL yet. */
const TERMS_HREF = "/kontak" as const;
...
function firstFieldMessage(fields: ResetFields, ...keys: ResetFieldKey[]): string | undefined {
...
export function ForgotPasswordForm({ className }: { className?: string }): ReactElement {
  ...
  const subtitle =
    step === "request"
      ? `Tenang — kami akan mengirim kode reset ke email akun ${BRAND_SHORT} Anda. Masukkan alamat yang terdaftar, lalu ikuti petunjuk di kotak masuk.`
      : step === "code"
        ? `Kode reset telah dikirim ke ${email.trim() || "email Anda"}. Masukkan kode tersebut untuk melanjutkan.`
        : "Pilih kata sandi baru yang kuat. Setelah disimpan, Anda akan masuk otomatis.";
  ...
  return (
    <div className={cn("w-full text-left", className)}>
      ...
      {/* Keep path discoverable for tests / tooling */}
      <span className="sr-only" data-auth-path={TEKNOVO_FORGOT_PASSWORD_PATH}>
        {TEKNOVO_FORGOT_PASSWORD_PATH}
      </span>
    </div>
  );
}
