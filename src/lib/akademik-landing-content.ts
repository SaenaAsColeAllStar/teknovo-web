...
export type AkademikSubNavSlug = "kurikulum" | "jurusan" | "pengajar";
...
export type AkademikOverviewTeaser = {
  iconKey: "kurikulum" | "jurusan" | "pengajar";
  title: string;
  description: string;
...
export const AKADEMIK_PAGE_LEDE =
  `${AKADEMIK_PAGE_INTRO} Halaman ini merangkum kurikulum Merdeka, jalur kelas X–XII, program kejuruan, dan komunitas pengajar ${BRAND_SHORT} untuk calon siswa dan orang tua.`;
...
export const AKADEMIK_SUB_NAV_ITEMS: readonly AkademikSubNavItem[] = [
  { id: "ringkasan", label: "Ringkasan", href: "/akademik", exact: true },
...
/** Accent for featured-feed category chips (compact rounded, not pill). */
export type AkademikFeedAccent = "brand" | "emerald" | "amber" | "violet";

export type AkademikFeedFeatured = {
  category: string;
...
export type AkademikFeedSideItem = {
  category: string;
  accent: AkademikFeedAccent;
  title: string;
  excerpt: string;
...
export type AkademikFeedKey = "ringkasan" | "kurikulum" | "jurusan" | "pengajar";

export type AkademikFeaturedFeedData = {
  featured: AkademikFeedFeatured;
  sideItems: readonly AkademikFeedSideItem[];
};
...
const akademikFeedAuthor = {
  authorName: BRAND_KEPALA_NAMA,
  authorRole: BRAND_KEPALA_JABATAN,
  authorAvatarSrc: BRAND_KEPALA_FOTO_SRC,
  publishedLabel: "Diperbarui semester ini",
} as const;

const akademikFeedArticles = {
  pengajar: {
    category: "Pengajar",
    accent: "amber" as const,
    title: PENGAJAR_SECTION_TITLE,
    excerpt: PENGAJAR_SECTION_INTRO[0],
    href: "/akademik/pengajar" as const,
    imageSrc: LANDING_MEDIA.fasilitas.lmsWebp,
    imageAlt: "Komunitas pengajar dan pembinaan akademik SMK TEKNOVO",
    ...akademikFeedAuthor,
  },
  kurikulum: {
    category: "Kurikulum",
    ...
    title: AKADEMIK_KURIKULUM_SECTION_TITLE,
    excerpt: AKADEMIK_KURIKULUM_SECTION_INTRO,
    href: "/akademik/kurikulum" as const,
    imageSrc: LANDING_MEDIA.fasilitas.laboratoriumWebp,
    imageAlt: "Praktik laboratorium dan jalur belajar SMK TEKNOVO",
    ...akademikFeedAuthor,
  },
  jurusan: {
    category: "Jurusan",
    ...
    title: JURUSAN_SECTION_TITLE,
    excerpt: JURUSAN_SECTION_INTRO[0],
    href: "/akademik/jurusan" as const,
    imageSrc: LANDING_MEDIA.akademik.jurusanTeknikMesinWebp,
    imageAlt: "Program kejuruan Teknik Mesin SMK TEKNOVO",
    ...akademikFeedAuthor,
  },
} as const;

function sideFrom(
  keys: readonly (keyof typeof akademikFeedArticles)[],
  excerpts: readonly string[],
): AkademikFeedSideItem[] {
  return keys.map((key, idx) => {
    const article = akademikFeedArticles[key];
    return {
      category: article.category,
      accent: article.accent,
      title: article.title,
      excerpt: excerpts[idx] ?? article.excerpt,
      href: article.href,
    };
  });
}
...
/**
 * Data-driven featured + side articles per akademik tab.
 * Ringkasan defaults to Tenaga pengajar as the featured story.
 */
export const AKADEMIK_FEATURED_FEEDS: Record<AkademikFeedKey, AkademikFeaturedFeedData> = {
  ringkasan: {
    featured: akademikFeedArticles.pengajar,
    sideItems: sideFrom(
      ["kurikulum", "jurusan"],
      [
        AKADEMIK_KURIKULUM_SECTION_INTRO,
        "Setiap jurusan dirancang dengan pembelajaran berbasis proyek dan praktik bengkel atau layanan.",
      ],
    ),
  },
  pengajar: {
  ...
  kurikulum: {
    featured: akademikFeedArticles.kurikulum,
    sideItems: sideFrom(
      ["jurusan", "pengajar"],
      [
        "Setiap jurusan dirancang dengan pembelajaran berbasis proyek dan praktik bengkel atau layanan.",
        "Guru mapel umum dan produktif dibimbing melalui komunitas belajar sekolah.",
      ],
    ),
  },
  jurusan: {
    featured: akademikFeedArticles.jurusan,
    sideItems: sideFrom(
      ["kurikulum", "pengajar"],
      [
        AKADEMIK_KURIKULUM_SECTION_INTRO,
...
/** Default feed for `/akademik` ringkasan (pengajar featured). */
export const AKADEMIK_FEATURED_FEED = AKADEMIK_FEATURED_FEEDS.ringkasan;

export function getAkademikFeaturedFeed(key: AkademikFeedKey = "ringkasan"): AkademikFeaturedFeedData {
  return AKADEMIK_FEATURED_FEEDS[key] ?? AKADEMIK_FEATURED_FEEDS.ringkasan;
}
