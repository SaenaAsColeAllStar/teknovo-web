...
import type { BreadcrumbList, SoftwareApplication, WebApplication, WebPage, WithContext } from "schema-dts";

import { BRAND_SCHOOL_FULL, BRAND_SHORT } from "@/lib/branding";
import { getFasilitasDetailPath } from "@/lib/fasilitas-landing-content";
...
export { PPDB_SCHOOL_LOCALITY, PPDB_SCHOOL_LOCATION, PPDB_SCHOOL_NPSN, PPDB_SCHOOL_REGION };
...
export function resolveLmsOgImageUrl(imagePath?: string): string {
  return resolveOgImageUrl(imagePath, LANDING_MEDIA.fasilitas.lmsWebp);
}
...
export function buildLmsPageMetadata(input: LmsPageSeoInput): Metadata {
  ...
  const ogImage = resolveLmsOgImageUrl(LANDING_MEDIA.fasilitas.lmsWebp);
      ...
      images: [
        {
          ...
          width: 1200,
          height: 630,
          alt: `LMS ${BRAND_SHORT} — pembelajaran online`,
        },
      ],
    },
    twitter: {
...
export function buildLmsDashboardMetadata(
  pageId: Extract<LmsPageId, "dashboard-login" | "guru-portal" | "kurikulum-hub">,
): Metadata {
...
export function buildLmsEducationalOrganizationJsonLd(): Record<string, unknown> {
  return buildPpdbEducationalOrganizationJsonLd();
}

export function buildLmsSoftwareApplicationJsonLd(): WithContext<SoftwareApplication> {
  ...
  return {
    ...
    alternateName: [
      "LMS SMK TEKNOVO",
      "Platform E-Learning SMK TEKNOVO",
      "Portal Pembelajaran Online TEKNOVO",
    ],
...
export function buildLmsWebApplicationJsonLd(): WithContext<WebApplication> {
  const portalUrl = buildConsoleAbsoluteUrl(LMS_PUBLIC_PATHS.portalLogin);
...
export function buildLmsWebPageJsonLd(input: {
  name: string;
  description: string;
  path: string;
  origin?: "landing" | "console";
  ...
  const url =
    input.origin === "console"
      ? buildConsoleAbsoluteUrl(input.path)
      : buildLandingAbsoluteUrl(input.path);
...
export function buildLmsBreadcrumbJsonLd(
  items: readonly LmsBreadcrumbItem[],
