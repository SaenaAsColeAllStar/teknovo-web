/**
 * Smoke-test Node API routes against local Postgres + MinIO (docker-compose).
 * Uses in-process Hono `app.request` (no listening port).
 *
 * Prerequisites:
 *   docker compose up -d
...
async function main() {
  ...
  const listKat = await app.request("/api/v1/kategori", {}, bindings);
  ...
  const listBerita = await app.request(
  ...
  const createAttempt = await app.request("/api/v1/kategori", {
    method: "POST",
  ...
  const mediaAttempt = await app.request("/api/cms/media", {}, bindings);
